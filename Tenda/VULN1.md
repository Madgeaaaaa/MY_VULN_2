...
![[2.png]]In the function, since the character corresponding to 126 is "~", but the list parameter normally does not contain this character, the for loop will not be entered.
![[3.png]]Subsequently, because neither "%[^,]" nor "%s" of sscanf specifies a maximum field width, it writes bytes beyond the buffer when encountering input that is longer than v12/v13/v14/v15, resulting in a buffer overflow vulnerability.
![[4.png]]
